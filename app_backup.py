...
def build_followup_recommendation_answer(user_text: str) -> Optional[str]:
    ...
    if is_size_question(user_text):
        ...
        if not user_size:
            return f"{idx}번으로 추천드린 {reco_context['product_name']} 기준으로 보려면 고객님 평소 {size_label} 사이즈를 먼저 알려주세요 :)"
        ...
        if size_eval["supported"] is False:
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {size_label} {user_size} 기준이면 넉넉하게 맞는 쪽으로 보긴 어려워요.\n"
        ...
        if size_eval["supported"] == "edge":
            ...
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {size_label} {user_size} 기준이면 가능권에는 들어오지만 경계선에 가까워요.\n"
        ...
        if size_eval["supported"] is True:
            if matched:
                return (
                    f"{idx}번으로 추천드린 {name}은 고객님 {size_label} {user_size} 기준이면 {matched['label']} 쪽으로 보시면 돼요 :)\n"
                    f"{reason or '현재 확인되는 범위 안쪽으로 보여요.'}\n"
                    "부담 없이 입는 쪽으로는 비교적 안정적인 편이에요."
                )
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {size_label} {user_size} 기준으로 현재 확인되는 권장 범위 안에 있어요 :)\n"
                f"{reason}\n"
...
def build_multi_recommendation_size_answer(user_text: str) -> Optional[str]:
    ...
    for i, reco in enumerate(targets, start=1):
        ...
        size_eval = evaluate_size_support(user_size, reco_context, reco_db)
        if size_eval["supported"] is True:
            lines.append(f"{i}번 {reco_context['product_name']}: 고객님 {size_label} {user_size} 기준으로는 무리 없는 편이에요.")
        elif size_eval["supported"] == "edge":
            lines.append(f"{i}번 {reco_context['product_name']}: 고객님 {size_label} {user_size} 기준으로 가능권이지만 경계선에 가까워요.")
        else:
            lines.append(f"{i}번 {reco_context['product_name']}: 고객님 {size_label} {user_size} 기준으로는 여유 있게 맞기 어려워 보여요.")
...
def build_size_answer(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> Optional[str]:
    ...
    if not user_size:
        return f"{size_label} 사이즈를 같이 입력해주시면 더 정확하게 봐드릴 수 있어요 :) 지금은 고객님 평소 {size_label} 사이즈를 먼저 알려주세요."
    ...
    if size_eval["supported"] is False:
        return (
            f"고객님 {size_label} {user_size} 기준이면 {current_name}은 넉넉하게 맞는 쪽으로 보긴 어려워요.\n"
            f"{reason or '현재 확인되는 사이즈 범위가 고객님보다 작게 잡혀 있어요.'}\n"
    ...
    if size_eval["supported"] == "edge":
        label = matched["label"] if matched else "현재 옵션"
        return (
            f"고객님 {size_label} {user_size} 기준이면 {current_name}은 가능권에는 들어오지만 경계선에 가까운 편이에요.\n"
            f"{reason or (label + ' 기준으로 상단 사이즈에 가까워 보여요.')}\n"
    ...
    if size_eval["supported"] is True:
        if matched:
            return (
                f"고객님 {size_label} {user_size} 기준이면 {matched['label']} 쪽으로 보시면 돼요 :)\n"
                f"{reason or ('현재 페이지 기준으로 ' + matched['label'] + '가 ' + matched['size_desc'] + ' 정도로 안내돼 있어요.')}\n"
                "다만 원하시는 핏이 슬림한지, 편하게 입는지에 따라 체감은 조금 달라질 수 있어요."
            )
        return (
            f"고객님 {size_label} {user_size} 기준이면 {current_name}은 현재 확인되는 권장 범위 안쪽으로 보여요 :)\n"
            f"{reason}\n"
            "그래도 여유 있게 입으시는 편이면 실측까지 함께 보는 쪽이 가장 안전해요."
        )
...
def process_user_message(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> None:
    ...
    if user_hash != st.session_state.get("last_logged_user_hash", ""):
        log_event("user_message", product_context, user_text=user_text)
        st.session_state.last_logged_user_hash = user_hash
    ...
    try:
        ...
        multi_followup_answer = build_multi_recommendation_size_answer(user_text)
        recommendation_answer = build_recommendation_answer(user_text, product_context, db_product)
        size_answer = build_size_answer(user_text, product_context, db_product)
        direct_answers = [
            multi_followup_answer,
            followup_answer,
        ...
        if answer is recommendation_answer:
            response_mode = "rule_recommendation"
        elif answer is size_answer:
            response_mode = "rule_size"
        elif answer is followup_answer or answer is multi_followup_answer:
        ...
        answer = "지금 답변을 정리하는 중에 잠깐 연결이 흔들렸어요. 같은 내용을 한 번만 다시 보내주시면 바로 이어서 봐드릴게요 :)"
        response_mode = "error"
        ...
        if answer:
            assistant_hash = str(hash(clean_text(answer)))
            if assistant_hash != st.session_state.get("last_logged_assistant_hash", "") or error_text:
                ...
                st.session_state.last_logged_assistant_hash = assistant_hash

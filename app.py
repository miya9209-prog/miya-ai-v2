...
def context_uses_top_size(product_context: Dict, db_product: Optional[Dict]) -> bool:
    ...
    if any(k in corpus for k in ["자켓", "재킷", "점퍼", "코트", "블라우스", "셔츠", "니트", "가디건", "맨투맨", "티셔츠", "후드", "조끼", "베스트"]):
        return True
    if any(k in corpus for k in ["팬츠", "슬랙스", "바지", "데님", "청바지", "스커트", "치마", "레깅스"]):
        return False
    return True
...
def evaluate_size_support(user_top: str, product_context: Dict, db_product: Optional[Dict]) -> Dict:
    ...
    if page_options:
        ...
        if not matched:
            return {
                "supported": False,
                "reason": f"최대 {rank_to_size(max_rank)} 정도까지로 보여요." if max_rank else "",
                "matched_option": None,
                "confidence": "page",
            }
        ...
        if chest_signal.get("status") is False:
            return {
                "supported": False,
                "reason": chest_signal.get("reason", "") or f"{matched['label']} 쪽은 타이트하게 느껴질 수 있어요.",
        ...
        if boundary or chest_signal.get("status") == "edge":
            return {
                "supported": "edge",
                "reason": chest_signal.get("reason", "") or f"{matched['label']}는 경계선에 가까운 쪽이에요.",
                "matched_option": matched,
                "confidence": "page+measure",
            }
        return {
            "supported": True,
            "reason": chest_signal.get("reason", "").strip(),
            "matched_option": matched,
            "confidence": "page+measure",
        }
    ...
    if ranks:
        ...
        if user_rank not in ranks:
            return {
                "supported": False,
                "reason": f"최대 {rank_to_size(max_rank)} 정도까지로 보여요.",
                "matched_option": None,
                "confidence": "db",
            }
        if chest_signal.get("status") is False:
            return {
                "supported": False,
                "reason": chest_signal.get("reason", "") or "실측상 타이트할 수 있어요.",
                "matched_option": None,
        ...
        if user_rank >= max_rank or chest_signal.get("status") == "edge":
            return {
                "supported": "edge",
                "reason": chest_signal.get("reason", "") or f"{user_top}까지는 가능해 보이지만 경계선에 가까워 보여요.",
                "matched_option": None,
                "confidence": "db+measure",
            }
        return {
...
def build_followup_recommendation_answer(user_text: str) -> Optional[str]:
    ...
    if is_size_question(user_text):
        ...
        if size_eval["supported"] is False:
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {body_label} {user_size} 기준이면 넉넉하게 맞는 쪽으로 보긴 어려워요.\n"
        ...
        if size_eval["supported"] == "edge":
            ...
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {body_label} {user_size} 기준이면 가능권에는 들어오지만 경계선에 가까워요.\n"
        ...
        if size_eval["supported"] is True:
            if matched:
                return (
                    f"{idx}번으로 추천드린 {name}은 고객님 {body_label} {user_size} 기준이면 {matched['label']} 쪽으로 보시면 돼요 :)\n"
                    f"{reason or '현재 확인되는 범위 안쪽으로 보여요.'}\n"
                    "부담 없이 입는 쪽으로는 비교적 안정적인 편이에요."
                )
            return (
                f"{idx}번으로 추천드린 {name}은 고객님 {body_label} {user_size} 기준으로 현재 확인되는 권장 범위 안에 있어요 :)\n"
                f"{reason}\n"
...
def recommend_products_for_query(user_text: str, current_product: Dict, body_ctx: Dict[str, str], limit: int = 3) -> List[Dict]:
    ...
    q = clean_text(user_text)
    seen_names = set(st.session_state.get("reco_seen_names", []) or []) if any(k in q for k in ["다른", "또", "말고", "없어"]) else set()
    ...
    for _, row in DB.iterrows():
        ...
        blob = " ".join([explicit, fit, style, cover, summary])
        ...
        if target == "자켓":
            ...
            if "가디건" not in explicit:
                continue
        elif target == "니트":
            if "니트" not in explicit:
                continue
        elif target == "스커트":
            if not any(k in explicit for k in ["스커트", "치마"]):
                continue
        elif target == "팬츠":
            if not any(k in explicit for k in ["팬츠", "슬랙스", "바지", "데님", "청바지"]):
                continue
        ...
        if current_product.get("style_tags"):
            ...
            row_tags = set(x for x in re.split(r"[;,/|]", style) if clean_text(x))
...
def build_size_answer(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> Optional[str]:
    ...
    has_regular = any(k in fit_corpus for k in ["정핏", "기본", "깔끔"])
    mentions_upper = any(k in q for k in ["상체", "어깨", "가슴"])
    mentions_petite = any(k in q for k in ["키가 작", "아담", "작은 편", "키 작은"])
    upper_rank = size_rank(user_size) or 0

    if body_label == "상의":
        upper_note = "상체가 있는 편이라고 하셔서 어깨나 가슴 쪽은 조금 더 또렷하게 느껴지실 수 있어요." if mentions_upper else ""
        petite_note = ""
        if mentions_petite:
            if has_loose:
                petite_note = "키가 아담하신 편이면 루즈핏 특성상 기장이나 품이 더 크게 느껴질 수 있어요."
            else:
                petite_note = "키가 아담하신 편이면 기장은 아주 과하게 길어 보이진 않는지 같이 보면 더 좋아요."

        if has_loose and mentions_upper and mentions_petite and upper_rank >= size_rank("77"):
            return " ".join([
                f"고객님 {body_label} {user_size} 기준으로 보면 {current_name}은 아예 못 입는 타입은 아니에요.",
                "다만 루즈핏이라 상체가 더 커 보이거나 전체적으로 조금 부해 보일 수는 있어요.",
                "키가 아담하신 편이면 기장이나 품도 실제보다 더 크게 느껴질 수 있고요.",
                "그래서 편하게 입는 건 가능하지만, 핏을 예쁘게 잡아 입는 기준이면 조금 더 깔끔한 정핏이나 기장 짧은 쪽을 같이 보시는 게 더 좋아요 :)",
            ])
        ...
        if size_eval["supported"] is False:
            ...
            if reason:
                parts.append(reason)
            if upper_note:
                parts.append(upper_note)
            if petite_note:
                parts.append(petite_note)
        ...
        if size_eval["supported"] is True:
            parts = [f"고객님 {body_label} {user_size} 기준이면 {current_name}은 크게 무리 없는 쪽으로 보여요 :)"]
            if reason:
                parts.append(reason)
            if upper_note:
                parts.append(upper_note)
            if petite_note:
                parts.append(petite_note)
            if has_regular:
                parts.append("전체적으로는 깔끔하게 잡히는 쪽에 더 가까워요.")
            elif has_loose:
                parts.append("다만 루즈핏이라 원하시는 느낌보다 조금 더 편한 쪽으로 느껴질 수는 있어요.")
            return " ".join(parts)

        parts = [f"{current_name}은 지금 정보만으로 아주 단정하게 잘 맞는다기보다, 편하게 입는 느낌 쪽에 가까울 수 있어요."]
        if upper_note:
            parts.append(upper_note)
        if petite_note:
            parts.append(petite_note)
        parts.append("핏을 예쁘게 잡아 입으시는 기준이면 비슷한 타입 중에 조금 더 깔끔한 쪽을 같이 보시는 게 좋아요 :)")
        return " ".join(parts)
    ...
    return "지금은 사이즈 정보가 또렷하게 잡히지 않아서, 실측표를 같이 보시는 쪽이 가장 안전해요 :)"
...
def build_recommendation_answer(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> Optional[str]:
    ...
    if not recos:
        st.session_state.last_recommendations = []
        return "지금 조건에 딱 맞는 상품이 바로 많이 잡히진 않아서요. 원하시면 자켓 느낌을 조금 더 단정하게 볼지, 편하게 볼지 기준을 맞춰서 다시 골라드릴게요 :)"
    ...
    st.session_state.last_reco_target = infer_target_category_from_query(user_text, current_product)
    ...
    if target == "자켓":
        ...
        opener = "네, 지금 옷이랑 잘 이어입기 좋은 바지로 먼저 골라드릴게요."
    else:
        opener = "네, 고객님 쪽에 잘 맞을 만한 쪽으로 먼저 골라드릴게요."
...
def is_greeting(user_text: str) -> bool:
    q = clean_text(user_text).replace(" ", "")
    return q in {"안녕", "안녕하세요", "하이", "반가워", "헬로"} or q.startswith("안녕")
...
def process_user_message(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> None:
    ...
    started = time.time()
    try:
        greeting_answer = "안녕하세요 :) 지금 보시는 상품 같이 봐드릴게요. 사이즈가 궁금하신지, 코디가 궁금하신지 편하게 말씀 주세요." if is_greeting(user_text) else None
        followup_answer = build_followup_recommendation_answer(user_text)
        direct_answers = [
            (greeting_answer, "rule"),
...
.block-container{max-width:760px;padding-top:0.01rem !important;padding-bottom:9.8rem !important;}
...
@media (max-width: 768px){.block-container{max-width:100%;padding-top:0.01rem!important;padding-bottom:10.2rem!important;}div[data-testid="stHorizontalBlock"]{gap:6px!important;}div[data-testid="stHorizontalBlock"] > div{flex:1 1 0!important;min-width:0!important;}div[data-testid="stChatInput"]{bottom:64px!important;width:calc(100% - 16px)!important;}}
</style>
...
st.markdown(
    ...
    <div style="margin-top:0; margin-bottom:0;">
      ...
      <div style="padding:4px 8px 0 8px; border:1px solid rgba(0,0,0,.04); border-radius:14px; background:transparent;">
    """,
    unsafe_allow_html=True,
)
...
if any(build_body_context().values()):
    st.markdown(f'<div style="margin-top:0; margin-bottom:0; font-size:10.8px; color:#7a7f8c;">현재 입력 정보: {html.escape(body_summary)}</div>', unsafe_allow_html=True)
...
for msg in st.session_state.messages:
    ...
    if msg["role"] == "user":
        st.markdown(
            '<div style="display:flex; justify-content:flex-end; width:100%; margin:0 0 8px 0;">'
            '<div style="max-width:92%;">'
            '<div style="display:block; font-size:12px; font-weight:700; line-height:1.15; color:#0f6a63; text-align:right; margin:0 6px 2px 0;">고객님</div>'
        ...
        st.markdown(
            '<div style="display:flex; justify-content:flex-start; width:100%; margin:0 0 8px 0;">'
            '<div style="max-width:92%;">'
            '<div style="display:block; font-size:12px; font-weight:700; line-height:1.15; color:#505767; text-align:left; margin:0 0 2px 6px;">미야언니</div>'

...
def log_event(event_type: str, data: Optional[Dict] = None) -> None:
    ...
    try:
        ...
        filename = os.path.join("logs", f"miya_log_{ym}.csv")
        row = {
            "timestamp": now.strftime("%Y-%m-%d %H:%M:%S"),
            "event_type": event_type,
            "session_id": st.session_state.get("session_id", ""),
            "product_no": clean_text(data.get("product_no", "")),
...
def call_llm(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> Optional[str]:
    ...
    last_error = None
    st.session_state["_last_latency_ms"] = 0
    for wait in (0, 1.2):
        if wait:
            time.sleep(wait)
        try:
            ...
            st.session_state["_last_latency_ms"] = int((time.time() - started) * 1000)
...
def process_user_message(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> None:
    ...
    current_product_no = normalize_product_no(clean_text((db_product or {}).get("product_no", "") or product_context.get("product_no", "")))
    current_product_name = clean_text((db_product or {}).get("product_name", "") or product_context.get("product_name", ""))
    ...
    try:
        ...
        if not answer:
            ...
            log_event("fallback", {
                ...
                "is_fallback": True,
                "latency_ms": st.session_state.get("_last_latency_ms", 0),
            })
        ...
        log_event("assistant_response", {
            ...
            "product_name": current_product_name,
            "user_text": user_text,
            "response_mode": response_mode or "rule",
            "fallback_reason": fallback_reason,

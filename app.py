...
def _rule_intent(user_text: str):
    ...
    if no_space in {"응", "네", "넵", "좋아", "그래", "ㅇㅇ", "어", "오케이", "ok", "ㅇㅋ"}:
        return "affirm"
    # 번호 선택 — "2번 셔츠처럼" 상품 지칭은 제외, 단독 선택만 처리
    if re.search(r"[123]번|첫 ?번째|두 ?번째|세 ?번째", q):
        _item_words = ["셔츠","바지","팬츠","니트","블라우스","자켓","원피스",
                       "스커트","아우터","코트","가디건","티셔츠","상품","거"]
        _is_item_ref = any(re.search(r"[123]번\s*" + w, q) for w in _item_words)
        if not _is_item_ref:
...
def fast_length_answer(user_text: str, current: Dict) -> str:
    """
    '길이 2가지 중 뭘로?' 같은 길이 선택 질문에 키 정보를 활용해 결론 있는 답변.
    """
    q = clean_text(user_text)
    # 길이 선택 질문 조건
    length_words = ["길이", "단", "롱", "숏", "짧은", "긴", "기장"]
    choice_words = ["뭘로", "어떤", "어느", "골라", "추천", "2가지", "두 가지", "선택"]
    if not any(w in q for w in length_words):
        return ""
    if not any(w in q for w in choice_words):
        return ""
    ...
    if height:
        try:
            h = int(height.replace("cm", "").strip())
            if h <= 160:
                return (f"{call}, 키 {h}cm이시면 {particle_eun_neun(name)} "
                        f"숏·단 기장을 먼저 추천드려요. "
                        f"롱은 발목까지 끌릴 수 있고, 짧은 기장이 다리가 길어 보이는 효과가 있어요. "
                        f"실내화보다 굽 있는 신발과 맞추시면 더 좋아요.")
            elif h <= 165:
                return (f"{call}, 키 {h}cm이시면 단·롱 모두 가능한 범위예요. "
                        f"신발 굽이 높으면 롱, 플랫이면 단 쪽이 더 안정적이에요. "
                        f"다리가 길어 보이길 원하시면 단 쪽에 굽 있는 신발 조합을 추천드려요.")
            else:
                return (f"{call}, 키 {h}cm이시면 {particle_eun_neun(name)} "
                        f"롱 기장도 예쁘게 소화하실 수 있어요. "
                        f"단은 발목이 깔끔하게 드러나서 경쾌하고, 롱은 우아한 느낌이에요. "
                        f"출근이나 격식 있는 자리면 롱, 데일리로는 단도 좋아요.")
        except (ValueError, AttributeError):
            pass

    # 키 정보 없을 때 일반 답변
    return (f"{call}, 길이 선택은 키와 신발 굽 높이를 같이 보시는 게 좋아요. "
            f"160cm 이하면 단 기장이 다리가 더 길어 보이고, "
            f"165cm 이상이면 롱도 예쁘게 소화하실 수 있어요. "

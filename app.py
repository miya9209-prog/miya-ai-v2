...
말투 규칙:
- 친근한 MD 상담체
- "옵션 중", "표기상", "기준으로 보면", "가능성이 높아요", "등록되어 있지 않다" 같은 기계적인 표현은 쓰지 않는다.
- "고객님 체형이면", "이 상품이랑 같이 입으시면", "이런 자리에는"처럼 자연스럽게 말한다.
- 답을 바로 먼저 말하고, 뒤에 이유를 붙인다.
- 상품명이 확실할 때만 상품명을 쓴다.
...
추천 답변 규칙:
- 추천 질문에는 일반론으로 돌리지 말고, 가능한 한 실제 상품명 2~3개를 먼저 제시한다.
- 추천 후보가 1개만 명확하면 그 1개를 먼저 말하고, 비슷한 결의 다른 후보도 1~2개 더 붙여준다.
- 추천 후보를 찾기 어려울 때도 "등록이 안 되어" 같은 표현 대신 "지금 DB에서 먼저 눈에 들어오는 건"처럼 부드럽게 말한다.
...
def parse_page_size_options(product_context: dict):
    ...
    for pat in patterns:
        for match in re.finditer(pat, text):
            ...
            size_desc = clean_text(match.group(0).split("(", 1)[1].rstrip(")")).replace("~", "-")
    ...
    general_patterns = [
        r"사이즈\s*TIP[^0-9가-힣A-Za-z]*(44|55반|55|66반|66|77반|77|88|99)\s*[-~]\s*(44|55반|55|66반|66|77반|77|88|99)",
        r"(44|55반|55|66반|66|77반|77|88|99)\s*[-~]\s*(44|55반|55|66반|66|77반|77|88|99)\s*(?:체형|사이즈)?(?:까지|가능|추천)",
        r"(44|55반|55|66반|66|77반|77|88|99)\s*체형까지",
        r"(44|55반|55|66반|66|77반|77|88|99)\s*까지\s*(?:추천|가능|착용)",
    ]
    for gp in general_patterns:
        for match in re.finditer(gp, text):
            groups = [g for g in match.groups() if g]
            if not groups:
                continue
            if len(groups) >= 2:
                size_desc = f"{groups[0]}-{groups[1]}"
            else:
                size_desc = f"55-{groups[0]}" if groups[0] != "44" else groups[0]
            ranks = expand_size_text(size_desc)
            if ranks:
                key = ("FREE", tuple(ranks))
                if key not in seen:
                    seen.add(key)
                    options.append({"label": "FREE", "size_desc": size_desc, "ranks": ranks})
...
def is_name_question(user_text: str):
    q = clean_text(user_text).replace(" ", "")
    keywords = ["이름", "상품명", "상품이름", "이옷이름", "이옷이뭐", "무슨옷", "품명"]
    return any(k in q for k in keywords)


def build_name_guard_answer(user_text: str, product_context: dict | None, db_product: dict | None):
    if not is_name_question(user_text):
        return None
    name = clean_text((db_product or {}).get("product_name", "")) or clean_text((product_context or {}).get("product_name", ""))
    if not name or name == "지금 보시는 상품":
        return "지금 보시는 상품명은 페이지 기준으로 한 번 더 확인해봐야 해서, 상세 상단에 적힌 이름 기준으로 같이 봐드릴게요 :)"
    summary = clean_text((db_product or {}).get("product_summary", "")) or clean_text((product_context or {}).get("summary", ""))
    category = clean_text((db_product or {}).get("category", "")) or clean_text((product_context or {}).get("category", ""))
    lines = [f"지금 보시는 상품은 {name}이에요 :)"]
    if summary:
        lines.append(summary[:120].strip())
    elif category and category != "기타":
        lines.append(f"미샵에서 보시는 {category} 쪽 상품으로 생각해주시면 돼요.")
    lines.append("궁금하신 점 있으시면 사이즈나 핏감도 바로 이어서 같이 봐드릴게요 :)")
    return "\\n".join(lines[:3])



def infer_target_category_from_query(user_text: str, current_product: dict | None = None):
    q = clean_text(user_text)
    mapping = [
        ("팬츠", ["바지", "슬랙스", "팬츠", "데님", "청바지"]),
        ("스커트", ["스커트", "치마"]),
    ...
    if any(k in q for k in ["어울", "같이 입", "코디", "매치"]):
        current_cat = clean_text((current_product or {}).get("category", ""))
        current_sub = clean_text((current_product or {}).get("sub_category", ""))
        corpus = f"{current_cat} {current_sub}"
        if any(k in corpus for k in ["셔츠", "블라우스", "니트", "티셔츠", "맨투맨", "가디건", "자켓", "아우터"]):
            return "팬츠"
        if any(k in corpus for k in ["팬츠", "슬랙스", "데님", "스커트"]):
            return "블라우스"
    return ""


def extract_preference_keywords(user_text: str):
    q = clean_text(user_text)
    keywords = [
        "슬랙스", "데님", "청바지", "와이드", "세미와이드", "일자", "부츠컷", "배기",
        "A라인", "정핏", "루즈핏", "단정", "학교", "방문", "출근", "하객", "데일리",
        "블랙", "네이비", "베이지", "아이보리", "그레이"
    ]
    return [k for k in keywords if k in q]


def row_text_blob(rowd: dict) -> str:
    cols = [
        "product_name", "category", "sub_category", "fit_type", "body_cover_features",
        "style_tags", "coordination_items", "product_summary", "recommended_body_type"
    ]
...
def category_aliases(target: str):
    ...
    return mapping.get(target, [target] if target else [])
...
def recommend_products_for_query(user_text: str, current_product: dict | None, body_ctx: dict, limit=3):
    ...
    preferred_words = extract_preference_keywords(user_text)
...
def build_size_guard_answer(user_text: str, product_context: dict | None, db_product: dict | None):
    ...
    if supported is False:
        ...
        lines = [
            ...
            "상세페이지 기준으로는 경계 밖에 가까워 보여서, 저는 무리해서 추천드리진 않을게요.",
        ]
        if recos:
            ...
            lines.append("이럴 땐 한 사이즈 더 여유 있게 나오는 상의 쪽으로 같이 보시는 게 더 안전해요.")
        return "\\n".join(lines)

    if supported is True and size_eval.get("matched_option"):
        opt = size_eval["matched_option"]
        return (
            f"고객님 상의 {user_top} 기준이면 {current_name}은 {opt['label']} 사이즈 쪽으로 보시면 가장 안정적이에요.\\n"
            f"페이지 기준으로 {opt['label']}가 {opt['size_desc']} 정도를 커버하는 쪽으로 안내돼 있어서 고객님 체형과 더 가깝게 맞을 가능성이 높아요.\\n"
            "너무 딱 붙는 느낌보다 편하게 입으실 수 있는 쪽으로 먼저 추천드릴게요 :)"
        )

    if supported is True:
        return (
            f"고객님 상의 {user_top} 기준이면 {current_name}은 현재 확인되는 범위 안에서는 가능 쪽으로 보여요.\\n"
            f"{size_eval.get('reason', '페이지나 DB 기준 권장 범위 안으로 잡혀요.')}\\n"
            "다만 실제로는 원하시는 핏감에 따라 체감이 달라질 수 있어서, 너무 딱 맞는 느낌보다 편한 핏 원하시면 실측도 같이 봐주시는 게 좋아요 :)"
        )

    return (
        f"지금 보시는 {current_name}은 사이즈 정보가 아예 없는 건 아닌데, 고객님 상의 {user_top}에 바로 확답드릴 만큼 또렷하게 잡히진 않았어요.\\n"
        "이럴 땐 상세페이지 사이즈 TIP과 실측표를 같이 보는 쪽이 제일 안전해요 :)"
    )
...
def build_color_guard_answer(user_text: str, product_context: dict | None, db_product: dict | None):
    ...
    if not colors:
        return "지금 페이지에서 컬러가 또렷하게 잡히지 않아서 없는 색을 제가 임의로 말씀드리긴 어려워요. 옵션창에 보이는 색상 기준으로 같이 봐드릴게요 :)"
...
def process_user_message(user_text: str, current_url: str, product_no_value: str, product_context: dict | None, db_product: dict | None):
    ...
    name_guard = build_name_guard_answer(user_text, product_context, db_product)
    if name_guard:
        st.session_state.messages.append({"role": "assistant", "content": name_guard})
        return

...
8. 추천 요청이 들어오면 현재 상품 사이즈 설명을 반복하지 말고, 추천 상품 후보만 골라서 답한다.
9. 상의 상품에서는 허리, 힙, 허벅지 같은 하의 중심 표현을 쓰지 않는다.
10. 하의 상품에서는 어깨, 가슴, 팔통 같은 상의 중심 표현을 쓰지 않는다.
""".strip()
...
def context_uses_top_size(product_context: Dict, db_product: Optional[Dict]) -> bool:
    ...
    if any(k in corpus for k in ["자켓", "재킷", "점퍼", "코트", "셔츠", "블라우스", "니트", "가디건", "맨투맨", "티셔츠", "탑", "조끼", "베스트"]):
        return True
...
def evaluate_size_support(user_top: str, product_context: Dict, db_product: Optional[Dict]) -> Dict:
    ...
    if page_options:
        ...
        return {
            "supported": True,
            "reason": f"현재 페이지 기준으로 {matched['label']} 사이즈가 고객님 {user_top}을 커버하는 쪽이에요. {chest_signal.get('reason','').strip()}".strip(),
            "matched_option": matched,
            "confidence": "page+measure",
        }
...
def is_recommendation_question(user_text: str) -> bool:
    ...
    return any(k in q for k in [
        "추천", "어울리는", "같이 입", "코디", "매치", "무슨 바지", "어떤 바지", "무슨 치마",
        "잘 어울리는", "다른", "비슷한", "학교에", "행사", "입고갈",
        "다른 맨투맨", "다른 자켓", "다른 블라우스", "다른 티셔츠", "다른 니트",
        "맨투맨 추천", "자켓 추천", "블라우스 추천", "티셔츠 추천", "니트 추천",
    ])
...
def infer_target_category_from_query(user_text: str, current_product: Dict) -> str:
    ...
    if "맨투맨" in q:
        return "맨투맨"
    if any(k in q for k in ["티셔츠", "티 ", "반팔", "긴팔티"]):
        return "티셔츠"
    ...
    if any(k in corpus for k in ["자켓", "재킷", "아우터", "점퍼", "코트", "맨투맨"]):
        return "팬츠"
...
def build_recommended_items_size_summary(user_text: str) -> Optional[str]:
    q = clean_text(user_text)
    recos = st.session_state.get("last_recommendations", []) or []
    if not recos or not is_size_question(user_text):
        return None
    if get_recommendation_reference_index(user_text) is not None:
        return None
    if not any(k in q for k in ["추천해준", "추천드린", "지금 추천", "방금 추천", "추천한", "그 바지", "그 상품", "그 옷"]):
        return None
    ...
    for i, reco in enumerate(recos[:3], start=1):
        ...
        if size_eval["supported"] is False:
            lines.append(f"{i}번 {name}은 고객님 {body_label} {user_size} 기준으로는 조금 타이트하게 느껴질 수 있어요.")
        elif size_eval["supported"] == "edge":
            lines.append(f"{i}번 {name}은 고객님 {body_label} {user_size} 기준으로 경계선에 가까운 편이에요.")
        elif size_eval["supported"] is True:
            lines.append(f"{i}번 {name}은 고객님 {body_label} {user_size} 기준으로 무리 없는 쪽이에요.")
    if not lines:
        return None
    return "\n".join(lines[:3]) + "\n마음 가는 번호 있으면 그 상품 기준으로 더 자세히 이어서 봐드릴게요 :)"
...
def build_followup_recommendation_answer(user_text: str) -> Optional[str]:
    group_size_answer = build_recommended_items_size_summary(user_text)
    if group_size_answer:
        return group_size_answer
...
def recommend_products_for_query(user_text: str, current_product: Dict, body_ctx: Dict[str, str], limit: int = 3) -> List[Dict]:
    ...
    for _, row in DB.iterrows():
        ...
        if target == "자켓":
            ...
            if not any(k in explicit for k in ["티셔츠", "반팔", "긴팔티"]):
                continue
        elif target == "스커트":
...
def build_recommendation_answer(user_text: str, product_context: Dict, db_product: Optional[Dict]) -> Optional[str]:
    ...
    if not recos:
        ...
        target = infer_target_category_from_query(user_text, current_product)
        if target == "맨투맨":
            return "지금 조건에 딱 맞게 바로 더 잡히는 맨투맨이 많진 않아요. 원하시면 조금 더 여유핏으로 볼지, 깔끔한 기본핏으로 볼지 기준 맞춰서 다시 골라드릴게요 :)"
        if target == "팬츠":
            return "지금 옷이랑 잘 붙는 바지 중에서 바로 더 잡히는 후보가 많진 않아요. 원하시면 슬랙스 쪽으로 볼지, 데님 쪽으로 볼지 기준 맞춰서 다시 골라드릴게요 :)"
        return "지금 조건에 딱 맞는 상품이 바로 많이 잡히진 않아서요. 원하시면 조금 더 단정하게 볼지, 편하게 볼지 기준을 맞춰서 다시 골라드릴게요 :)"
